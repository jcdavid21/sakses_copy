...
<body>
    ...
    <script>
        // 1. ADD THESE GLOBAL VARIABLES (replace existing global variables section)
        let enrollments = <?php echo json_encode($enrollments); ?>;
        let filteredEnrollments = [...enrollments];
        let currentPage = 1;
        const recordsPerPage = 10;

        // 2. UPDATE THE DOCUMENT READY FUNCTION
        $(document).ready(function() {
            ...
            initializeFilters();
            initializeSearch();
            filterEnrollments(); // This will render the table with pagination
        });

        // 5. UPDATE THE initializeSearch FUNCTION
        ...
        // 6. UPDATE THE initializeFilters FUNCTION
        ...
        // 3. UPDATE THE filterEnrollments FUNCTION
        ...
        // 7. UPDATE THE clearFilters FUNCTION
        ...
        // 8. ADD NEW PAGINATION FUNCTIONS
    ...
    `);
        ...
        // 4. UPDATE THE renderEnrollmentsTable FUNCTION
        ...
        `);
            ...
            const endIndex = Math.min(startIndex + recordsPerPage, filteredEnrollments.length);
            const currentPageData = filteredEnrollments.slice(startIndex, endIndex);

            // Render current page data
            currentPageData.forEach(enrollment => {
        ...
        function updatePaginationInfo() {
            ...
            const endIndex = Math.min(currentPage * recordsPerPage, filteredEnrollments.length);

            $('#showingStart').text(startIndex);
            $('#showingEnd').text(endIndex);
            $('#totalRecords').text(filteredEnrollments.length);
        }
